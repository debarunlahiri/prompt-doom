...
function editor(config, item, reload) {
  openModal(
    ...
    `<form id="taxonomy-form" class="taxonomy-form"><label class="taxonomy-name">Name<input name="name" value="${escapeHtml(item?.name || "")}" placeholder="Enter ${config.singular} name" required></label><label class="taxonomy-status">Status<select class="select-control" name="status"><option ${item?.status !== "inactive" ? "selected" : ""}>active</option><option ${item?.status === "inactive" ? "selected" : ""}>inactive</option></select></label>${config.id === "categories" ? `<label class="taxonomy-description">Description<textarea name="description" placeholder="Describe this category">${escapeHtml(item?.description || "")}</textarea></label>` : ""}<div class="form-actions"><button type="button" class="button" data-close-modal>Cancel</button><button class="primary">${icon("save")} Save</button></div></form>`,
